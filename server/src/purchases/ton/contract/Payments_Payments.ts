...
export function storeDepositSuccess(src: DepositSuccess) {
    return (builder: Builder) => {
        const b_0 = builder;
...
export function loadDepositSuccess(slice: Slice) {
    const sc_0 = slice;
    if (sc_0.loadUint(32) !== 3667243021) { throw Error('Invalid prefix'); }
    const _queryId = sc_0.loadUintBig(64);
    const _paymentId = sc_0.loadStringRefTail();
    const _payer = sc_0.loadAddress();
    return { $$type: 'DepositSuccess' as const, queryId: _queryId, paymentId: _paymentId, payer: _payer };
}

export function loadTupleDepositSuccess(source: TupleReader) {
    const _queryId = source.readBigNumber();
...
export function loadGetterTupleDepositSuccess(source: TupleReader) {
    const _queryId = source.readBigNumber();
    const _paymentId = source.readString();
    const _payer = source.readAddress();
    return { $$type: 'DepositSuccess' as const, queryId: _queryId, paymentId: _paymentId, payer: _payer };
...
export function storeTupleDepositSuccess(source: DepositSuccess) {
    const builder = new TupleBuilder();
    builder.writeNumber(source.queryId);
...
export function dictValueParserDepositSuccess(): DictionaryValue<DepositSuccess> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(beginCell().store(storeDepositSuccess(src)).endCell());
        },
        parse: (src) => {
            return loadDepositSuccess(src.loadRef().beginParse());
        }
    }
}
...
export function loadPayments$Data(slice: Slice) {
    const sc_0 = slice;
...
export function dictValueParserPayments$Data(): DictionaryValue<Payments$Data> {
    return {
        serialize: (src, builder) => {
            builder.storeRef(beginCell().store(storePayments$Data(src)).endCell());
        },
        parse: (src) => {
            return loadPayments$Data(src.loadRef().beginParse());
        }
    }
}

 type Payments_init_args = {
    $$type: 'Payments_init_args';
    owner: Address;
    uniqueId: bigint;
}

function initPayments_init_args(src: Payments_init_args) {
    return (builder: Builder) => {
        const b_0 = builder;
        b_0.storeAddress(src.owner);
        b_0.storeInt(src.uniqueId, 257);
...
export const gasForTransfer = 10000000n;

export class Payments implements Contract {
    
    public static readonly storageReserve = 0n;
    public static readonly errors = Payments_errors_backward;
    public static readonly opcodes = Payments_opcodes;
    
    static async init(owner: Address, uniqueId: bigint) {
        return await Payments_init(owner, uniqueId);
    }
    
    static async fromInit(owner: Address, uniqueId: bigint) {
        const __gen_init = await Payments_init(owner, uniqueId);
        const address = contractAddress(0, __gen_init);
        return new Payments(address, __gen_init);
    }
    
    static fromAddress(address: Address) {
        return new Payments(address);
    }
    
    readonly address: Address; 
    readonly init?: { code: Cell, data: Cell };
    readonly abi: ContractABI = {
        types:  Payments_types,
        getters: Payments_getters,
        receivers: Payments_receivers,
